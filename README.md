...
インストーラーは検出した Discord と確認項目を表示し、上下矢印キーと Enter で選択できます。`curl ... | bash` で起動した場合も、ターミナルが使える環境では同じ矢印選択になります。
...
対象のターミナルが矢印選択に対応していない場合は、従来の番号入力や `Y/n` 入力に切り替わります。プロンプトなしで実行したい場合は、後述の `DMI_NONINTERACTIVE=1` を使ってください。
