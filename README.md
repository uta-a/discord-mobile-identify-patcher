...
GUI と自動更新機能はまだありません。インストール/アンインストールは CLI と Windows/macOS スクリプトで行います。
...
自作 mobile patch だけを外す場合は、既定の `self` を使います。Vencord がある場合は Vencord を残します。
...
Vencord 層だけを外して自作 mobile patch を残す場合は `DMI_UNINSTALL_TARGET=vencord-layer` を指定します。

```powershell
$env:DMI_UNINSTALL_TARGET = "vencord-layer"; powershell -NoProfile -ExecutionPolicy Bypass -Command "irm https://raw.githubusercontent.com/uta-a/discord-mobile-identify-patcher/main/scripts/bootstrap-uninstall-windows.ps1 | iex"; Remove-Item Env:\DMI_UNINSTALL_TARGET
```

```bash
curl -fsSL https://raw.githubusercontent.com/uta-a/discord-mobile-identify-patcher/main/scripts/bootstrap-uninstall-macos.sh | DMI_UNINSTALL_TARGET=vencord-layer bash
```
...
node src/cli.mjs uninstall-self --branch stable --force-close
node src/cli.mjs uninstall-vencord-layer --branch stable --force-close
```
